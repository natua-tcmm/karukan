...
- 変換学習（全文履歴に加え、明示修正した文節を左右の文脈付きで記憶）
...
1. 📝 文節学習（現在の読みと左右ヒントが一致する場合）
2. 📝 全文学習キャッシュ
3. 👤 ユーザー辞書
4. 🤖 モデル推論・モデルと辞書の合成候補
5. 📚 システム辞書ラティス
6. ひらがな / カタカナ
7. 🔄 Rewriter（半角カタカナ・英字全角半角・記号バリアント）
...
明示変換では、候補移動・数字/クリック選択・文節境界変更を行った文節だけを別キャッシュへ記録します。初期候補をそのままEnterで確定した文節や、ライブ変換をそのまま確定した内容は文節学習の対象になりません。
...
- 文節学習だけを削除するには: `rm ~/.local/share/karukan-im/segment_learning.tsv`
