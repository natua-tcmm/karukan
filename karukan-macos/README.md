...
- 現在のプロトコルはv2。候補クリックは現在の文節へ適用し、Enterで全文を確定
- キーイベントはエンジン内部のXKB keysym表現に変換して送る
...
新しい形式のアーカイブに辞書ライセンスとソースマニフェストが含まれる場合は、
同じデータディレクトリへ併せて保存します。上流v0.1.0のように`docs/`へ
ライセンス文書が入っている旧形式も、`dictionary-licenses/`として保存します。

既定では上流リポジトリの辞書を取得します。独自に監査・公開した辞書を使う場合は、
インストール時にURLを差し替えられます。

```bash
make install DICT_URL=https://example.com/releases/dict.tgz
```
...
| 右⌘ 単独タップ | 日本語入力(ひらがな)へ戻る。直接入力モード、Shift+英字で入った英字モードのどちらからでも有効。⌘Cなど他のキーと組み合わせた場合は発動しない |
...
生成するアプリには、Karukan本体のMIT License、Rust依存ライブラリ、Mozc由来データ、
形態素解析辞書、変換モデルのライセンス表示を同梱します。独立した配布物を公開する
前に、[再配布チェックリスト](../docs/redistribution.md)を確認してください。
