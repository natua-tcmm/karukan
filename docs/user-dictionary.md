...
入力中の読み全体だけでなく、文中に含まれる登録語もライブ変換へ反映されます。たとえば、`かるかん` → `karukan` を登録している場合、`かるかんをつかう` は `karukanを使う` のように変換されます。

一致した区間にはユーザー辞書の第1候補が固定され、その前後だけがAIモデルで変換されます。登録表記は後続区間のコンテキストにも使われます。AI推論の完了待ち中も登録語は即座に反映され、未変換の前後部分は読みのまま表示されます。

複数の登録語が重なる場合は、登録語で覆える文字数が最大になる組み合わせを選びます。同じ文字数なら、分割数が少ない長い登録語、辞書スコアが高い候補の順に優先されます。

文中一致は読みが2文字以上の登録語に限定されます。1文字の登録語は、読み全体が完全一致した場合にだけ従来どおり利用されます。また、1つの読みに複数の表記がある場合、第1候補をライブ本文へ使い、別表記を使った全文を変換候補へ表示します。
